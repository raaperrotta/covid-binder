...
def guessed_param_priors(guess):
    sd = 1.0
    nonzero = 0.01
    LOG.debug('Generating Lognormal priors for guessed values with sd=%f and %f in plce of zeros.', sd, nonzero)
    return [
        pm.Lognormal(k, mu=guess[k] or nonzero, sd=sd, testval=guess[k] or nonzero)
...
def main():
    with open('linearfit2.pkl', 'rb') as f:
        guess = pickle.load(f)[0]
    with create_model(guess):
